...
const Login = () => {
  ...
  const { signInUser, googleSignInUser, resetPasswordUser } =
  ...
  const handleGoogleSignInUser = () => {
    googleSignInUser()
      .then((result) => {
        const user = result.user;
        toast.success("Google Login Successfull", { autoClose: 1000 });
        navigate(from, { replace: true });
      })
      .catch((error) => {
        toast.error(error.message, { autoClose: 1000 });
  ...
  const handleResetPassword = () => {
    resetPasswordUser(userEmail)
      .then(() => {
        toast.success("Send Reset Password Link Your Email Address....", {
          autoClose: 1000,
        });
      })
      .catch((error) => {
